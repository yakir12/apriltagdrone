...
using VideoIO, OpenCV, StaticArrays
...
file = "/home/yakir/mnt/dacke_lab_data/Data/Yakir/a.mp4"
...
_, i = findmax(quality, tags)

function findHomography(src, dst)
    mask = Matrix{Float64}(undef, 3, 3)
    h, g = OpenCV.findHomography(OpenCV.Mat(reshape(reinterpret(Float32, src), 2, 1, 4n)), OpenCV.Mat(reshape(reinterpret(Float32, dst), 2, 1, 4n)), OpenCV.RANSAC, 5.0, OpenCV.Mat(reshape(mask, 1, 3, 3)), 2000, 0.995)
    Matrix(reshape(h, 3 ,3))
end
